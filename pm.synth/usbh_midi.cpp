...
uint8_t MIDI::Init(uint8_t parent, uint8_t port, bool lowspeed) {
	...
	if (!isMidiFound) { //MIDI Device not found. Try first Bulk transfer device
		...
		epInfo[epDataOutIndex].maxPktSize =
				epInfo[epDataOutIndexVSP].maxPktSize;
	}
	...
	bPollEnable = true;
	return 0;
	FailGetDevDescr: FailSetDevTblEntry: FailGetConfDescr: FailSetConfDescr: Release();
	return rcode;
}
	...
	while (buf_ptr < buf + total_length) {
		...
		case USB_DESCRIPTOR_INTERFACE:
			if (buf_ptr[5] == USB_CLASS_AUDIO
					&& buf_ptr[6] == USB_SUBCLASS_MIDISTREAMING) { //p[5]; bInterfaceClass = 1(Audio), p[6]; bInterfaceSubClass = 3(MIDI Streaming)
...
//          bConfNum = 0;
		...
		case USB_DESCRIPTOR_ENDPOINT:
			...
			if ((epDesc->bmAttributes & 0x02) == 2) { //bulk
				uint8_t index;
				if (isMidi)
					index = ((epDesc->bEndpointAddress & 0x80) == 0x80) ?
							epDataInIndex : epDataOutIndex;
				else
					index = ((epDesc->bEndpointAddress & 0x80) == 0x80) ?
							epDataInIndexVSP : epDataOutIndexVSP;
...
uint8_t MIDI::RcvData(uint16_t *bytes_rcvd, uint8_t *dataptr) {
	...
	return pUsb->inTransfer(bAddress, epInfo[epDataInIndex].epAddr, bytes_rcvd,
			dataptr);
}
...
uint8_t MIDI::RcvData(uint8_t *outBuf) {
	...
	RcvData_return_from_buffer: readPtr++;
